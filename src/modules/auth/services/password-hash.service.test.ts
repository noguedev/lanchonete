...
describe("PasswordHash", () => {
  ...
  it("validates the correct password as true", async () => {
    ...
    await expect(service.validate(hash, password)).resolves.toBe(true);
  });

  it("validates an incorrect password as false", async () => {
    const hash = await service.hash(password);

    await expect(service.validate(hash, "senha-errada")).resolves.toBe(false);
  });

  it("rethrows errors for an invalid hash", async () => {
    await expect(service.validate("hash-invalido", password)).rejects.toThrow();
  });

  it("rejects an empty password during verification", async () => {
    const hash = await service.hash(password);

    await expect(service.validate(hash, "")).resolves.toBe(false);
    await expect(service.validate(hash, "")).not.toBe(true);
  });
});
